        ...
        <div className="grid grid-cols-2 gap-4 md:hidden">
          {stats.map((stat) => {
            ...
            return (
              <div
                key={stat.label}
                className="flex flex-col items-center gap-2 rounded-2xl border border-brand-navy-950/10 bg-white p-5 text-center"
              >
                <Icon className="h-5 w-5 text-brand-orange-500" />
                <p className="text-xl font-bold text-brand-navy-950">{stat.value}</p>
                <p className="text-xs font-medium text-brand-navy-900/70">{stat.label}</p>
              </div>
            );
          })}
        </div>
